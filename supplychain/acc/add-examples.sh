...
import-files() {
    ...
    for i in ${!GENERATORS_FILES[@]}; do
        acc generator create-from-file --file=supplychain/acc/examples/generators/${GENERATORS_FILES[$i]}.yaml
    done

    for i in ${!ACCELERATORS_FILES[@]}; do
        acc accelerator create-from-file --file=supplychain/acc/examples/accelerators/${ACCELERATORS_FILES[$i]}.yaml
    done
}
