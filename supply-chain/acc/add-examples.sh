...
add-spring-examples() {
    ...
    ACCELERATORS_FILES=("music-store-steeltoe" "ebanking" "todo-service" "function-kafka" "spring-jpa" "ruby-simple" "dotnet-aspnet-hello" "boot-function-knative" "frontend-ux-for-online-stores" "backend-api-for-online-stores" "supply-chain-for-online-stores")
...
import-files() {
    ...
    for i in ${!GENERATORS_FILES[@]}; do
        acc generator create-from-file --file=supply-chain/acc/examples/generators/${GENERATORS_FILES[$i]}.yaml
    done

    for i in ${!ACCELERATORS_FILES[@]}; do
        acc accelerator create-from-file --file=supply-chain/acc/examples/accelerators/${ACCELERATORS_FILES[$i]}.yaml
    done
}
